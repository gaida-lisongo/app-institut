...
export default function EnrollmentDetailsPage({
    data,
    onBack,
}: { data: Recherche; onBack?: () => void }) {  
  console.log('Recherche reçue:', data);
  const [recherche, setRecherche] = useState<Recherche | null>(data);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">{recherche.title}</p>
          <p className="text-sm text-gray-500">{recherche.promotionId?.designation} - {recherche.anneeId?.debut}-{recherche.anneeId?.fin}</p>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {recherche.subscribers?.map((subscriber, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">
                      {subscriber.student?.nom} {subscriber.student?.prenom}                    
                      <p>{subscriber.student?.matricule}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <a 
                        className=''
                        href={subscriber.report ?? '#'} target="_blank" rel="noopener noreferrer"
                    >
                        {subscriber?.title || '-'}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-mono">
                    ...
                    <span>{subscriber?.tuteur?.grade} | {subscriber?.tuteur?.email}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
